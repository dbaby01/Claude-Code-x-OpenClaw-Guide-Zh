...
> **2026-04 差量更新**
>
> - **v2026.4.5** 新增实验性 Dreaming（做梦）记忆整理系统：light/deep/REM 三阶段后台整理，Recall Decay 记忆老化控制，Dream Diary UI
> - **v2026.4.5** Dreaming 产出写入 `dreams.md`，与每日日志分开
> - **v2026.4.24** 混合记忆搜索现在暴露原始 `vectorScore` 和 `textScore`；会话记录搜索支持可见性和 Agent 间策略
