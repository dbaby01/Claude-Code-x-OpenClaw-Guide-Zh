...
>
> - **v2026.3.31**：新增 QQ Bot 内置支持（多账号、斜杠命令、媒体）
> - **v2026.4.8**：修复多个捆绑 Channel 启动问题（BlueBubbles、飞书、Google Chat、IRC、Matrix 等）
> - **v2026.4.14**：Telegram Forum Topics 显示话题名称；Slack/Discord/Teams 安全增强
> - **v2026.4.22**：WhatsApp 原生引用回复、每群/每 DM 独立 systemPrompt；Matrix 交叉签名验证
> - **v2026.4.24**：Google Meet 捆绑参与者插件，支持实时语音循环
