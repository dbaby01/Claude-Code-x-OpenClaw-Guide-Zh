...
- `/review` 当前仍是官方命令；已移除的是 `/pr-comments`（v2.1.91）和 `/vim`（v2.1.92）。
...
### v2.1.93 → v2.1.133 重要新功能

以下是 v2.1.92 之后引入的、与日常使用直接相关的新功能（版本号经 GitHub Release Notes 逐条核实）：

#### Monitor 工具（v2.1.98+）

后台监控器，持续跟踪指定事件并将结果实时送入当前对话。

```text
# 在 Claude Code 中请求监控
帮我启动一个 monitor，每 30 秒检查 build 状态
```
...
Claude 会在后台持续运行监控，有重要事件时自动插入对话。

#### Session Recap（v2.1.114+）

当你的终端窗口失去焦点再回来时（比如切去浏览器查资料、或被同事叫走），Claude Code 会自动显示一个简短的 **recap**：在你离开期间发生了什么。

- 不需要任何配置，自动生效
- 显示内容：完成的工具调用、生成的文件、重要的对话输出
- 帮你快速恢复上下文，不用滚动翻历史

#### 自定义主题（v2.1.114+）

通过 `/theme` 命令或 Plugin 构建自定义颜色方案：
...
- 内置多种配色方案
- Plugin 可以声明自定义颜色主题（color palette）
- 支持系统暗/亮模式自动适配

#### Native Binaries（v2.1.105-113 Week 16）

Claude Code CLI 从 Node.js 脚本迁移到了**原生二进制文件**。你可能注意到的变化：

- 启动速度更快
- 安装不再需要全局 npm
- 下载安装包更简洁

> 💡 **迁移说明**：如果你之前用 `npm install -g @anthropic-ai/claude-code` 安装，现在推荐使用官方安装脚本（见 01-安装指南）。旧版 npm 安装方式仍然可用，但原生二进制是推荐路径。

#### Opus 4.7 默认模型（v2.1.105-113 Week 16）

Max 和 Team Premium 订阅用户的默认模型已升级为 **Opus 4.7**。配合 `xhigh` effort 级别使用可获得最佳推理效果。
