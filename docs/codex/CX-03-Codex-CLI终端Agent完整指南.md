...
|------|------|
| `/help` | 查看当前可用命令 |
| `/permissions` | 调整 Codex 在本会话中可自动执行的动作范围 |
| `/sandbox-add-read-dir` | Windows 下给沙箱增加额外只读目录 |
| `/agent` | 切换或查看当前 subagent 线程 |
| `/apps` | 浏览连接器并插入到提示词中 |
| `/plugins` | 浏览、安装或管理插件 |
| `/clear` | 清空终端并开始新对话 |
| `/compact` | 压缩当前对话，释放上下文 |
| `/copy` | 复制最近一次 Codex 输出 |
| `/diff` | 查看 Git diff，包括未跟踪文件 |
| `/exit` / `/quit` | 退出 CLI |
| `/experimental` | 开关实验性功能 |
| `/feedback` | 发送日志或反馈给维护者 |
| `/init` | 在当前目录生成 `AGENTS.md` 脚手架 |
| `/logout` | 退出 Codex 登录 |
| `/mcp` | 查看 MCP 工具；可加 verbose 看服务详情 |
| `/mention` | 把文件或目录附加到当前对话 |
| `/model` | 查看或切换可用模型 |
| `/fast` | 切换 Fast mode |
| `/plan` | 切换到计划模式，可直接附带任务描述 |
| `/goal` | 实验性长目标工作流：让 Codex 围绕一个可验证目标持续推进 |
| `/personality` | 切换回应风格 |
| `/ps` | 查看实验性后台终端和最近输出 |
| `/stop` | 停止当前会话启动的后台终端 |
| `/fork` | 分叉当前对话 |
...
| `/new` | 在同一 CLI 会话中新建对话 |
| `/review` | 审查当前工作区改动 |
| `/status` | 查看会话配置、模型、审批、上下文等状态 |
| `/debug-config` | 输出配置层级和 requirements 诊断 |
| `/statusline` | 交互式配置 TUI 状态栏 |
| `/title` | 交互式配置终端窗口/标签标题 |
| `/keymap` | 查看并持久化 TUI 快捷键绑定 |

不要把 slash commands 写成永远固定的完整表。教程可以列当前官方命令，但应提醒用户以当前 CLI 的 `/help` 和 slash popup 为准。`/approvals` 仍可作为别名使用，但官方说明它不再出现在 slash popup 列表中。

### 7.1.1 `/goal`：长目标工作流

`/goal` 适合“比一次普通提示更长、但有明确停止条件”的任务，例如大迁移、长重构、部署重试循环、原型持续打磨。它不是提醒器，也不是后台定时任务；关键是写清楚目标、边界、验证方式和停止条件。

当前官方文档把 `/goal` 标为实验性 Codex CLI 能力。若当前 CLI 看不到它，先用 `/experimental` 启用，或在 `config.toml` 的 `[features]` 下设置 `goals = true`。常用控制包括：
...
A：进入 CLI 后用 `/help` 看当前列表。常见有 `/help`、`/model`、`/approvals`、`/mcp`、`/status`、`/diff`、`/compact`、`/new`、`/goal` 等；完整列表以当前 CLI 和官方 Slash Commands 文档为准。`/goal` 属于实验性长目标工作流，如果看不到，先检查 `/experimental` 或 `[features].goals`。
